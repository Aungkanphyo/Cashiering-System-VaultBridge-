          ...
          <table className="w-full">
            ...
            <tbody className="text-sm font-semibold text-slate-600">
              {currentData.map((sale) => (
                <tr
                    key={sale.id} className={`border-t hover:bg-slate-50 ${
                    sale.status === "VOIDED" ? "bg-slate-100 text-slate-400 line-through" : ""}`}
                >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="w-96 rounded-xl bg-white p-6 shadow-xl">
              ...
              <div className="mt-6 flex justify-end gap-3">
                <button onClick={() => setSaleToDelete(null)} className="rounded-lg bg-gray-500 px-4 py-2 text-white">Cancel</button>
                ...
                <button
                  ...
                  className="rounded-lg bg-red-500 px-4 py-2 text-white hover:bg-red-600">Delete
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {Array.from({ length: totalPages }).map((_, index) => (
                <button
                    ...
                    page === index + 1 ? "bg-green-600 text-white" : "border hover:bg-slate-100"}`}
                >
                    {index + 1}
                </button>
            ))}
