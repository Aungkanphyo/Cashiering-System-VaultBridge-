...
class StaffController extends Controller
{
    ...
    public function show(int $id)
    {
        ...
        if ($staff->join_date !== '-') {
            ...
            $durationParts = [];
            if ($diff->y > 0) $durationParts[] = $diff->y . ' ' . ($diff->y > 1 ? 'Years' : 'Year');
            if ($diff->m > 0) $durationParts[] = $diff->m . ' ' . ($diff->m > 1 ? 'Months' : 'Month');
            if ($diff->d > 0 || empty($durationParts)) $durationParts[] = $diff->d . ' ' . ($diff->d > 1 ? 'Days' : 'Day');
